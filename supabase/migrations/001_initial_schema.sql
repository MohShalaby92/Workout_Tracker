...
CREATE INDEX idx_template_sections_template ON template_sections(workout_template_id);
...
CREATE INDEX idx_template_exercises_section ON template_exercises(section_id);
...
CREATE INDEX idx_workout_logs_template    ON workout_logs(workout_template_id) WHERE workout_template_id IS NOT NULL;
...
CREATE INDEX idx_logged_sets_log ON logged_sets(workout_log_id);
-- Critical composite index for PR lookups: find all sets for a client+exercise
CREATE INDEX idx_logged_sets_client_exercise ON logged_sets(workout_log_id, template_exercise_id);
...
CREATE INDEX idx_personal_records_client_exercise ON personal_records(client_id, exercise_id);
CREATE INDEX idx_personal_records_exercise_1rm    ON personal_records(client_id, exercise_id, estimated_1rm DESC);
...
CREATE INDEX idx_injuries_client ON injuries(client_id);
CREATE INDEX idx_injuries_active ON injuries(client_id) WHERE status = 'active';
...
CREATE INDEX idx_skills_client ON skills(client_id);
...
CREATE INDEX idx_videos_coach    ON videos(coach_id);
CREATE INDEX idx_videos_exercise ON videos(exercise_id) WHERE exercise_id IS NOT NULL;
...
-- Epley formula: estimated_1rm = weight_kg * (1 + reps / 30)
-- Inserts a new personal_records row when the set beats the
-- existing best estimated 1RM for that client + exercise.
...
BEGIN
  -- Only process sets that have weight and reps
  IF NEW.weight_kg IS NULL OR NEW.reps IS NULL OR NEW.reps < 1 THEN
    RETURN NEW;
  END IF;

  -- Resolve client_id from the parent workout_log
  ...
  -- Resolve exercise_id from the template_exercise
...
-- Check if a user is the coach of a given client
...
-- Get IDs of all programs owned by the current coach
CREATE OR REPLACE FUNCTION my_program_ids()
RETURNS SETOF UUID AS $$
  SELECT id FROM programs WHERE coach_id = auth.uid()
$$ LANGUAGE sql STABLE SECURITY DEFINER;

-- Get IDs of all programs assigned to the current client
...
CREATE POLICY "users_coach_reads_clients"
  ON users FOR SELECT
...
CREATE POLICY "cc_coach_all"
  ON coach_clients FOR ALL
  USING (coach_id = auth.uid());

CREATE POLICY "cc_client_read"
...
CREATE POLICY "wt_coach_all"
  ON workout_templates FOR ALL
  USING (program_id IN (SELECT my_program_ids()));

CREATE POLICY "wt_client_read"
...
CREATE POLICY "exercises_read_global"
  ON exercises FOR SELECT
  USING (is_global = true);

CREATE POLICY "exercises_coach_own"
  ON exercises FOR ALL
  USING (created_by = auth.uid());

CREATE POLICY "exercises_client_read_coach"
  ON exercises FOR SELECT
  USING (
    created_by IN (
...
CREATE POLICY "wl_client_all"
  ON workout_logs FOR ALL
  USING (client_id = auth.uid());

CREATE POLICY "wl_coach_read"
...
CREATE POLICY "pr_client_all"
  ON personal_records FOR ALL
  USING (client_id = auth.uid());

CREATE POLICY "pr_coach_read"
...
CREATE POLICY "injuries_client_all"
  ON injuries FOR ALL
  USING (client_id = auth.uid());

CREATE POLICY "injuries_coach_all"
  ON injuries FOR ALL
  USING (is_my_client(client_id));
...
CREATE POLICY "skills_client_all"
  ON skills FOR ALL
  USING (client_id = auth.uid());

CREATE POLICY "skills_coach_all"
  ON skills FOR ALL
  USING (is_my_client(client_id));
...
CREATE POLICY "videos_coach_all"
  ON videos FOR ALL
  USING (coach_id = auth.uid());
